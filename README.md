# Assignment 5 – LoRA Fine-tuning & IBM ART Adversarial Attacks
...
- docker run --gpus -it -v $(pwd)/weights:/app/weights -v $(pwd)/data:/app/data dlops-ass5 bash   (buid and run docker)


## Q1 – ViT-S LoRA Fine-tuning on CIFAR-100
(here, wdbkey = a wandb generated key)

**Running Instruction :**
- cd Q1
- python train.py --mode all --wandb_key YOUR_WANDB_KEY    (Run all experiments)
- python train.py --mode baseline --wandb_key wdbkey     (Run baseline only)
- python train.py --mode lora --rank 4 --alpha 8 --dropout 0.1 --wandb_key wdbkey    (Run a single LoRA config)
...
**Running Instruction :**
- cd Q2
- python fgsm.py --train --epochs 50 --wandb_key wdbkey   (Train ResNet18 from scratch)
- python fgsm.py --ckpt ../weights/resnet18_cifar10_best.pt --wandb_key wdbkey    (Evaluate FGSM attacks)
